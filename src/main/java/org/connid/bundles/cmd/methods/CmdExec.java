...
public abstract class CmdExec {
    ...
    protected void exec(final String path, String [] env) {
        try {
            doExec(path, env);
        } catch (Exception e) {
    ...
    private void doExec(final String path, final String[] env) throws IOException {
        CmdConnection cmdConnection = CmdConnection.openConnection();
        String result = cmdConnection.execute(path, env);
